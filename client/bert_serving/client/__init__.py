...
from .DiversityHelper import DiversityHelper
import numpy as np
import zmq
...
ResponsePAGE = namedtuple('Response', ['id','tokens'])


class BertClient(object):
    def __init__(self, ip='localhost', port=5555, port_out=5556,
                 output_fmt='ndarray', show_server_config=False,
                 identity=None, check_version=False, check_length=False,
                 check_token_info=True, ignore_all_checks=True,
                 timeout=-1):
    ...
    def _recv_resPAGE(self, wait_for_req_id=None):
        request_id, response = self._recv(wait_for_req_id)
        res = jsonapi.loads(response[1])
        return ResponsePAGE(id=request_id, tokens=res)
    ...
    def encode(self, texts, blocking=True, is_tokenized=False, show_tokens=False):
        ...
        # res_punc=[DiversityHelper.beamSeachDeduplicationByPunc(each) for each in r.tokens["tokens"]]
        # print(texts)
        # res_ter = []
        # for input, ouputs in zip(texts,res_punc):
        #     res_ter.append(DiversityHelper.sort_by_TER(input,ouputs))
        #
        # res_bleu = []
        # for input, ouputs in zip(texts,res_punc):
        #     res_bleu.append(DiversityHelper.sort_by_bleu(input,ouputs))
        #
        # return {"TER":res_ter,"BLEU":res_bleu}
        # return {"RES": res_punc}
        return {"res": r.tokens["tokens"]}
